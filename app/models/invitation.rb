class Invitation < ApplicationRecord
  ...
  def self.update_users_topic_after_invite_accept(invitee_user_id, invited_user_id, assignment_id)
    new_team_id = TeamParticipant.team_id(assignment_id, invitee_user_id)
    ...
    original_team_id = TeamParticipant.team_id(assignment_id, invited_user_id)
    if original_team_id
      # team_participant_mapping = TeamParticipant.where(team_id: original_team_id, user_id: invited_user_id).first
      team_participant_mapping = TeamParticipant.find_by(team_id: original_team_id, user_id: invited_user_id)
      TeamParticipant.update(team_participant_mapping.id, team_id: new_team_id)
    else
      TeamParticipant.create(team_id: new_team_id, user_id: invited_user_id)
    end
  end
  ...
  # Last the users team entry will be added to the TeamParticipant table and their assigned topic is updated
  def self.accept_invitation(team_id, inviter_user_id, invited_user_id, assignment_id)
    ...
    if TeamParticipant.team_empty?(team_id) && (team_id != '0')
    ...
    can_add_member = TeamParticipant.add_member_to_invited_team(inviter_user_id, invited_user_id, assignment_id)
