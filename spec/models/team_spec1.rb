describe Team do
  ...
  let(:permissions) { OpenStruct.new(can_mentor: true) }
  let(:participant) { build(:participant, permissions: permissions, user: build(:student, name: 'Jane', fullname: 'Doe, Jane', id: 1)) }
#   let(:participant) { build(:participant, user: build(:student, name: 'Jane', fullname: 'Doe, Jane', id: 1)) }
  ...
  before(:each) do
    ...
    allow(participant).to receive(:can_mentor).and_return(true)
  end
